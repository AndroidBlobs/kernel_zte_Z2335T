			      ...
			      struct tiny_short_config *tiny_short_config)
{
	...
	if (cts_get_tiny_short_data(cts_dev, tiny_short_config) == 1) {
		...
		cts_log.log_cnt += snprintf(buf + cts_log.log_cnt, PAGE_SIZE - cts_log.log_cnt,
				"short threshold:%d\n", th);
		/* rx data */
		cts_log.log_cnt += snprintf(buf + cts_log.log_cnt, PAGE_SIZE - cts_log.log_cnt, "rx_logic_data:");
...
#define RAWDATA_BUFFER_SIZE(cts_dev) \
	...
	count += snprintf(buf + count, PAGE_SIZE - count, "th_min:%d[0x%x], th_max: %d[0x%x]\n",
	...
	if (is_open_test && ng_num) {
		...
		count += snprintf(buf + count, PAGE_SIZE - count, "TX open test...\n");
		for (i = 0; i < cts_dev->fwdata.rows; i++) {
			result[i] = 0;
			for (j = 0; j < cts_dev->fwdata.cols; j++) {
				if ((rawdata[i * cts_dev->fwdata.cols + j] < th_min)
		...
		count += snprintf(buf + count, PAGE_SIZE - count, "RX open test...\n");
		for (j = 0; j < cts_dev->fwdata.cols; j++) {
...
int cts_rawdata_test(struct cts_device *cts_dev, u16 th_min, u16 th_max)
{
	...
	if (ret < 0) {
		...
		snprintf(buf + cts_log.log_cnt, PAGE_SIZE - cts_log.log_cnt, "!!! Cts rawdata test error !!!\n");
	} else if (ret > 0) {
		cts_info("!!! Cts rawdata test FAIL  !!!");
		snprintf(buf + cts_log.log_cnt, PAGE_SIZE - cts_log.log_cnt, "!!! Cts rawdata test FAIL !!!\n");
	} else {
		cts_info("!!! Cts rawdata test PASS  !!!");
		snprintf(buf + cts_log.log_cnt, PAGE_SIZE - cts_log.log_cnt, "!!! Cts rawdata test PASS !!!\n");
	}
...
int cts_open_test(struct cts_device *cts_dev, u16 th)
{
	...
	if (ret < 0) {
		...
		snprintf(buf + cts_log.log_cnt, PAGE_SIZE - cts_log.log_cnt, "!!! Cts open test error !!!\n");
	} else if (ret > 0) {
		cts_info("!!! Cts open test FAIL  !!!");
		snprintf(buf + cts_log.log_cnt, PAGE_SIZE - cts_log.log_cnt, "!!! Cts open test FAIL !!!\n");
	} else {
		cts_info("!!! Cts open test PASS  !!!");
		snprintf(buf + cts_log.log_cnt, PAGE_SIZE - cts_log.log_cnt, "!!! Cts open test PASS !!!\n");
	}
