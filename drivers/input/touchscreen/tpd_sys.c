		...
		int len)
{
	...
	if (cdev->tp_i2c_reg_read) {
		cdev->tp_i2c_reg_read(cdev, addr, data, len);
	}
	return 0;
}
		...
		int len)
{
	...
	if (cdev->tp_i2c_reg_write) {
		cdev->tp_i2c_reg_write(cdev, addr, data, len);
	}
	return 0;
}
static int tp_16breg_read(
		u16 addr,
		u8 *data,
		int len)
{
	...
	if (cdev->tp_i2c_16breg_read) {
		cdev->tp_i2c_16breg_read(cdev, addr, data, len);
	}
	return 0;
}
static int tp_16breg_write(
		u16 addr,
		u8 *data,
		int len)
{
	...
	if (cdev->tp_i2c_16breg_write) {
		cdev->tp_i2c_16breg_write(cdev, addr, data, len);
	}
	return 0;
}
	...
	if (tp_reg_rw.len < 0) {
		...
		if (tp_reg_rw.type == REG_OP_READ) {
			if (tp_reg_rw.res == 0) {
				...
				count =
				    snprintf(buf, PAGE_SIZE, "Read reg%02X failed, ret: %d\n", tp_reg_rw.reg,
					     tp_reg_rw.res);
			}
		} else {
			if (tp_reg_rw.res == 0) {
				count =
				    snprintf(buf, PAGE_SIZE, "Write reg%02X, %02X success\n", tp_reg_rw.reg,
					     tp_reg_rw.val);
			} else {
				count =
				    snprintf(buf, PAGE_SIZE, "Write reg%02X failed, ret: %d\n", tp_reg_rw.reg,
					     tp_reg_rw.res);
			}
		}
	} else {
		if (tp_reg_rw.type == REG_OP_READ) {
			...
			if (tp_reg_rw.res) {
				...
				if (tp_reg_rw.opbuf) {
					for (i = 0; i < tp_reg_rw.len; i++) {
						count +=
						    snprintf(buf + count, PAGE_SIZE, "%02X ", tp_reg_rw.opbuf[i]);
					}
			...
			if (tp_reg_rw.opbuf) {
				for (i = 0; i < tp_reg_rw.len; i++) {
					count += snprintf(buf + count, PAGE_SIZE, "%02X ", tp_reg_rw.opbuf[i]);
				}
				count += snprintf(buf + count, PAGE_SIZE, "\n");
			}
			if (tp_reg_rw.res) {
				...
				const char __user *buffer, size_t len, loff_t *off)
{
	...
	pr_info("cmd len: %d, buf: %s", (int)cmd_length, buf);
	/* compatible old ops */
	if (cmd_length == tpd_fw_cdev.reg_char_num) {
		tp_reg_rw.type = REG_OP_READ;
		tp_reg_rw.len = 1;
		if (tpd_fw_cdev.reg_char_num == 2)
			tp_reg_rw.reg = str_to_int(buf, 2);
		else if (tpd_fw_cdev.reg_char_num == 4)
			tp_reg_rw.reg = str_to_int(buf, 4);
		...
		if (tp_reg_rw.type == REG_OP_READ) {
			if (tp_reg_rw.len == 1) {
				...
				if (tpd_fw_cdev.reg_char_num == 2) {
					...
					tp_reg_rw.res = tp_16breg_read(tp_reg_rw.reg, &val, 1);
			}
				tp_reg_rw.val = val;
			} else {
				reg = tp_reg_rw.reg & 0xFF;
				if (tpd_fw_cdev.reg_char_num == 2) {
					...
					tp_reg_rw.res =  tp_16breg_read(tp_reg_rw.reg, tp_reg_rw.opbuf, tp_reg_rw.len);
				}
			}

			if (tp_reg_rw.res < 0) {
				pr_info("Could not read 0x%02x", tp_reg_rw.reg);
			} else {
				pr_info("read 0x%02x, %d bytes successful", tp_reg_rw.reg, tp_reg_rw.len);
				tp_reg_rw.res = 0;
			}
			...
			if (tp_reg_rw.len == 1) {
				...
				if (tpd_fw_cdev.reg_char_num == 2) {
					...
					tp_reg_rw.res = tp_16breg_write(tp_reg_rw.reg, &val, 1);
				}
			} else {
				reg = tp_reg_rw.reg & 0xFF;
				if (tpd_fw_cdev.reg_char_num == 2) {
					...
					tp_reg_rw.res =  tp_16breg_write(tp_reg_rw.reg, tp_reg_rw.opbuf, tp_reg_rw.len);
				}
			}
			if (tp_reg_rw.res < 0) {
				pr_info("Could not write 0x%02x", tp_reg_rw.reg);

			} else {
				pr_info("Write 0x%02x, %d bytes successful", tp_reg_rw.reg, tp_reg_rw.len);
				tp_reg_rw.res = 0;
			}
		}
	}
...
static void create_tpd_proc_entry(void)
{
	...
	tpd_proc_entry = proc_create("rw_reg", 0664,  tpd_proc_dir, &proc_ops_rwreg);
	if (tpd_proc_entry == NULL)
		pr_notice("proc_create rw_reg!\n");
}
...
static int __init tpd_class_init(void)
{
	...
	tpd_fw_cdev.tp_i2c_16breg_read = NULL;
	tpd_fw_cdev.tp_i2c_16breg_write = NULL;
	tpd_fw_cdev.b_gesture_enable = 0;
	tpd_fw_cdev.TP_have_registered = false;
	tpd_fw_cdev.reg_char_num = 2;
	/**for tpd test*/
