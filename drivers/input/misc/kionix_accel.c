...
/*KIONIX calibrate definition*/
#define KIONIX_CAL_USING_SNS_CLASS
#ifdef KIONIX_CAL_USING_SNS_CLASS
#define KIONIX_CAL_SKIP_CNT		(2)
#define KIONIX_CAL_MAX			(10 + KIONIX_CAL_SKIP_CNT)
#define RAW_TO_1G				(16384)
#define RAW_TO_2G				(32767)
#define KIONIX_CAL_DELAY		(60) /*ms*/
#define KIONIX_X_AXIS_THRESHOLD	(0.2 * RAW_TO_1G)
#define KIONIX_Y_AXIS_THRESHOLD	(0.2 * RAW_TO_1G)
#define KIONIX_Z_AXIS_THRESHOLD	(0.3 * RAW_TO_1G)
...
#define KXTJ3_POWER_UP_TIME_MS 30
...
static void kionix_accel_grp4_calibrate_accel_xyz(struct kionix_accel_driver *acceld,
						s16 *acc_data_s16)
{
	s32 acc_data_s32[3] = {0};

	acc_data_s32[0] = acc_data_s16[0] - acceld->cal_params[0];
	acc_data_s32[1] = acc_data_s16[1] - acceld->cal_params[1];
	acc_data_s32[2] = acc_data_s16[2] - acceld->cal_params[2];
	if (abs(acc_data_s32[0]) > RAW_TO_2G) {
		acc_data_s32[0] = (acc_data_s32[0] > 0 ? RAW_TO_2G : -RAW_TO_2G);
	}
	if (abs(acc_data_s32[1]) > RAW_TO_2G) {
		acc_data_s32[1] = (acc_data_s32[1] > 0 ? RAW_TO_2G : -RAW_TO_2G);
	}
	if (abs(acc_data_s32[2]) > RAW_TO_2G) {
		acc_data_s32[2] = (acc_data_s32[2] > 0 ? RAW_TO_2G : -RAW_TO_2G);
	}
	acc_data_s16[0] = (s16)acc_data_s32[0];
	acc_data_s16[1] = (s16)acc_data_s32[1];
	acc_data_s16[2] = (s16)acc_data_s32[2];
}

static int kionix_accel_grp4_read_accel_xyz(struct kionix_accel_driver *acceld,
						s16 *acc_data_s16)
{
	int loop = KIONIX_I2C_RETRY_COUNT;
	int  err = -1;
	...
	while (loop) {
		...
		if (err < 0) {
			...
			loop = 0;
			dev_dbg(&acceld->client->dev, "succed to get accel data!\n");
		}
	}

	if (acceld->use_cal) {
		kionix_accel_grp4_calibrate_accel_xyz(acceld, acc_data_s16);
	}
...
/*	struct input_dev *input_dev = acceld->input_dev;*/
	...
	if (atomic_read(&acceld->accel_enabled) > 0) {
		if (atomic_read(&acceld->accel_enable_resume) > 0) {
			ts = ktime_get_boottime();
#ifndef KIONIX_CAL_USING_SNS_CLASS
			loop = KIONIX_I2C_RETRY_COUNT;
			while (loop) {
				mutex_lock(&input_dev->mutex);
				err =
...
#else
			err = kionix_accel_grp4_read_accel_xyz(acceld, acc_data_s16);
#endif
			if (err < 0) {
				KMSGERR(&acceld->client->dev,
					"%s: read data output error = %d\n",
				...
				acceld->accel_data[acceld->axis_map_z] =
				    ...
				    acceld->accel_cali[acceld->axis_map_z];
#else
				accel_data.accel_data_s16[0] = acc_data_s16[0];
				accel_data.accel_data_s16[1] = acc_data_s16[1];
				accel_data.accel_data_s16[2] = acc_data_s16[2];
				x = ((s16)
				     le16_to_cpu(accel_data.accel_data_s16
						 [acceld->axis_map_x]));
				y = ((s16)
...
static int kionix_accel_enable(struct kionix_accel_driver *acceld)
{
	...
	if (acceld->do_power_on_init) {
		acceld->do_power_on_init = false;

		/* do sensor power on init */
...
static int kionix_accel_platform_hw_power_on(bool on)
{
	...
	if (data->power_on != on) {
		...
		if (err)
			...
			data->power_on = on;
		if(err == 0 && data->power_on == true) {
			data->do_power_on_init = true;
			msleep(KXTJ3_POWER_UP_TIME_MS);
		}
	}
		...
		s16 *cal_xyz)
{
	...
	KMSGINF(&acceld->client->dev, "%s: begin to get data:\n", __func__);

	for (i = 0; i < KIONIX_CAL_MAX; i++) {
		msleep(KIONIX_CAL_DELAY);
		err = kionix_accel_grp4_read_accel_xyz(acceld, acc_data_s16);
		...
		if (i < KIONIX_CAL_SKIP_CNT) {
			KMSGINF(&acceld->client->dev,
					"%s: skip the %d accel data=(%d,%d,%d)\n",
					__func__, i,  acc_data_s16[0],  acc_data_s16[1],  acc_data_s16[2]);
			continue;
		}
		KMSGINF(&acceld->client->dev,
				"%s: get the %d accel data=(%d,%d,%d)\n",
				__func__, i,  acc_data_s16[0],  acc_data_s16[1],  acc_data_s16[2]);

		sum[0] += acc_data_s16[0];
		sum[1] += acc_data_s16[1];
		sum[2] += acc_data_s16[2];
	}

	cal_xyz[0] = sum[0] / (KIONIX_CAL_MAX -
			KIONIX_CAL_SKIP_CNT);
	cal_xyz[1] = sum[1] / (KIONIX_CAL_MAX -
			KIONIX_CAL_SKIP_CNT);
	cal_xyz[2] = sum[2] / (KIONIX_CAL_MAX -
			KIONIX_CAL_SKIP_CNT);
	KMSGINF(&acceld->client->dev,
			"%s: sum=(%d,%d,%d), avg=(%d,%d,%d)\n",
...
static int kionix_calibrate(struct sensors_classdev *sensors_cdev,
		int axis, int apply_now)
{
	...
	KMSGINF(&client->dev, "%s: enter\n", __func__);

	acceld->use_cal = false;
	pre_enable = atomic_read(&acceld->accel_enabled);
	if (pre_enable) {
		kionix_accel_cdev_enable(sensors_cdev, false);
	}
	pre_delay = atomic_read(&acceld->delay);
	KMSGINF(&client->dev, "%s: pre_delay=%d\n", __func__, pre_delay);
	atomic_set(&acceld->delay, 50); /*50ms*/

	kionix_accel_cdev_enable(sensors_cdev, true);

	if (atomic_cmpxchg(&acceld->cal_status, 0, 1)) {
		KMSGERR(&client->dev, "%s:do calibration error\n", __func__);
		err = -EBUSY;
		goto exit;
	}

	err = kionix_axis_calibrate(acceld, xyz);
	if (err < 0) {
		KMSGERR(&client->dev, "%s: error\n", __func__);
		goto cali_error;
	}

	KMSGINF(&client->dev, "%s: axis=%d\n", __func__, axis);
	switch (axis) {
	case AXIS_X:
		xyz[1] = 0;
		xyz[2] = 0;
	...
	case AXIS_XYZ:
		...
		break;
	default:
		KMSGERR(&client->dev, "%s :can not calibrate accel\n", __func__);
		break;
	}
	KMSGINF(&client->dev, "%s: offset=(%d,%d,%d)\n",
				__func__, xyz[0], xyz[1], xyz[2]);

	if (abs(xyz[0]) > (int)(KIONIX_X_AXIS_THRESHOLD) ||
		abs(xyz[1]) > (int)(KIONIX_Y_AXIS_THRESHOLD) ||
		abs(xyz[2]) > (int)(KIONIX_Z_AXIS_THRESHOLD)) {
		KMSGERR(&client->dev, "%s: failed for exceeding threshold!\n", __func__);
		err = -1;
		goto cali_error;
	}
	...
	if (apply_now) {
		acceld->cal_params[0] = xyz[0];
		acceld->cal_params[1] = xyz[1];
		acceld->cal_params[2] = xyz[2];
		acceld->use_cal = true;
		sensors_cdev->params = acceld->calibrate_buf;
	}

cali_error:
	atomic_set(&acceld->cal_status, 0);

exit:
	kionix_accel_cdev_enable(sensors_cdev, false);
	if (pre_enable) {
		atomic_set(&acceld->delay, pre_delay);
		kionix_accel_cdev_enable(sensors_cdev, true);
	}
	KMSGINF(&client->dev, "%s: end(%d)\n", __func__, err);
	return err;
}
		...
		struct cal_result_t *cal_result)
{
	...
	acceld->cal_params[0] = cal_result->offset_x;
	acceld->cal_params[1] = cal_result->offset_y;
	acceld->cal_params[2] = cal_result->offset_z;

	snprintf(acceld->calibrate_buf, sizeof(acceld->calibrate_buf),
			"%d,%d,%d", acceld->cal_params[0],
			acceld->cal_params[1], acceld->cal_params[2]);

	sensors_cdev->params = acceld->calibrate_buf;
	acceld->use_cal = true;
	KMSGINF(&acceld->client->dev, "%s:read accel calibrate bias %d,%d,%d\n",
		__func__, acceld->cal_params[0], acceld->cal_params[1], acceld->cal_params[2]);
...
#ifdef KIONIX_CAL_USING_SNS_CLASS
	acceld->cal_params[0] = 0;
	acceld->cal_params[1] = 0;
	acceld->cal_params[2] = 0;
	acceld->use_cal = false;
#endif
...
#ifdef KIONIX_CAL_USING_SNS_CLASS
	acceld->cdev.sensors_calibrate = kionix_calibrate;
	acceld->cdev.sensors_write_cal_params = kionix_write_cal_params;
	acceld->cdev.params = acceld->calibrate_buf;
#endif
	err = sensors_classdev_register(&acceld->input_dev->dev, &acceld->cdev);
	if (err) {
		KMSGERR(&client->dev, "%s:create class device file failed\n", __func__);
		err = -EINVAL;
