...
static int sx9310_suspend(struct device *dev)
{
	...
	input = chip->pinput;*/
	pr_err("%s enter\n", __func__);
	rc = pinctrl_select_state(chip->pinctrl, chip->sx9310_sleep);
	if (rc) {
		pr_err("%s Can't select pinctrl default state\n",__func__);
	}

	if (chip) {
	pr_err("%s report far when diabling sar\n", __func__);
...
static int sx9310_resume(struct device *dev)
{
	...
	rc = pinctrl_select_state(chip->pinctrl, chip->sx9310_active);
	if (rc) {
		pr_err("%s Can't select pinctrl default state\n",__func__);
	}
