...
static int ltr559_esd_check(void)
{
	int ret = 0;
	uint8_t buffer[4] = {0};
	...
	buffer[0] = LTR559_ALS_CONTR;
	ret = I2C_Read(buffer, 4);
	LTR559_DBG("%s:ALS_CONTR = %d, LTR559_PS_N_PULSES = %d.\n", __func__, buffer[0], buffer[3]);
	if (((buffer[0] & 0x1c) == 0) || ((buffer[3] & 0x0F) == 0x01)) {
		LTR559_DBG("%s:ltr559 esd check error.\n", __func__);
		ret = -EIO;
	} else {
		ret = 0;
	}
			...
			unsigned int enable)
{
	...
	if ((atomic_read(&ltr559_resume_poweron_flag) == 1) && (enable)) {
		...
		if (enable && !data->enable_ps_sensor && (ltr559_esd_check() < 0))
			ltr559_poweron_reg_config();
	}
...
#elif defined(CONFIG_BOARD_WARD)
	if (prox_mean < 1200) {
		prox_threshold_hi_param = prox_mean + 160;
		prox_threshold_lo_param = prox_mean + 90;
	} else {
