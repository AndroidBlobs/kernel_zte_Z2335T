...
static struct dsi_cmd_desc backlight_cmd_cabc53_jd9161[] = {
{
	{DTYPE_DCS_LWRITE, 1, 0, 0, 1, sizeof(led_bf_open)},	led_bf_open},
	{{DTYPE_DCS_WRITE1, 1, 0, 0, 1, sizeof(led_cabc53)},	led_cabc53},
	{{DTYPE_DCS_LWRITE, 1, 0, 0, 1, sizeof(led_bf_close)},	led_bf_close}
};
#endif
...
/*add by yujianhua set cabc 53=24 before enter sleep for JD9161B*/
	...
	if (level == 0) {
		...
		if ((strnstr(&pinfo->panel_name[0], "zteHELITAI_jd9161_hsd_480_854_5.0Inch",
			...
			MDSS_MAX_PANEL_LEN) != NULL)) {
			cmdreq.cmds = backlight_cmd_cabc53_jd9161;
			cmdreq.cmds_cnt = 3;
		} else {
			cmdreq.cmds = &backlight_cmd_cabc53;
			cmdreq.cmds_cnt = 1;
...
/*ZTE modify for lcd common function end */
	pr_info("[MSM_LCD]%s: ctrl_pdata->bklt_ctrl=%d\n", __func__, ctrl_pdata->bklt_ctrl);
	...
	if (data) {
		if (!strcmp(data, "bl_ctrl_wled")) {
			...
			pr_debug("%s: Configured DCS_CMD bklt ctrl\n",
								__func__);
		/*ZSW_MODIFY begin,add for P809A50 lcd driver,yangchaofeng,20151214*/
		} else if (!strncmp(data, "bl_ctrl_gpio", 12)) {
			ctrl_pdata->bklt_ctrl = BL_CMD_GPIO;
		/*ZSW_MODIFY end,add for P809A50 lcd driver,yangchaofeng,20151214*/
