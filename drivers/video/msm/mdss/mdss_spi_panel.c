...
int mdss_spi_panel_reset(struct mdss_panel_data *pdata, int enable)
{
	...
	if (!gpio_is_valid(ctrl_pdata->disp_dc_gpio)) {
		pr_info("%s:%d, dc line not configured\n",
			   __func__, __LINE__);
		return rc;
	}

	pr_info("spi %s: enable = %d\n", __func__, enable);
	pinfo = &(ctrl_pdata->panel_data.panel_info);

	if (enable) {
		rc = gpio_request(ctrl_pdata->rst_gpio, "disp_rst_n");
		if (rc)
			pr_err("display reset gpio (%d) request failed\n", ctrl_pdata->rst_gpio);
		else
			pr_info("display reset gpio (%d) request successful\n", ctrl_pdata->rst_gpio);

		rc = gpio_request(ctrl_pdata->disp_dc_gpio, "disp_dc");
		if (rc)
			pr_err("display dc gpio (%d) request failed\n", ctrl_pdata->disp_dc_gpio);
		else
			pr_info("display dc gpio (%d) request successful\n", ctrl_pdata->disp_dc_gpio);
...
static int mdss_spi_panel_power_on(struct mdss_panel_data *pdata)
{
	...
	ctrl_pdata = container_of(pdata, struct spi_panel_data,
				panel_data);
	if (gpio_is_valid(ctrl_pdata->panel_power_enable)) {
		gpio_set_value((ctrl_pdata->panel_power_enable), 1);
		usleep_range(2000, 3000);
	} else {
		pr_err("%s:%d, SPI anel_power_enable not configured\n",
			   __func__, __LINE__);
	}
...
static int mdss_spi_panel_power_off(struct mdss_panel_data *pdata)
{
	...
	if (gpio_is_valid(ctrl_pdata->panel_power_enable)) {
		usleep_range(2000, 3000);
		gpio_set_value((ctrl_pdata->panel_power_enable), 0);
	} else {
		pr_err("%s:%d, SPI anel_power_enable not configured\n",
			   __func__, __LINE__);
	}
...
static unsigned char spi_write_ram[] = {0x2c};

int mdss_spi_panel_write_ram(struct spi_panel_data *ctrl_pdata)
{
	mutex_lock(&ctrl_pdata->spi_tx_mutex);
	gpio_set_value(ctrl_pdata->disp_dc_gpio, 0);
	mdss_spi_tx_command(&spi_write_ram[0]);
			...
			char *buf, int len, int dma_stride)
{
	...
	mdss_spi_panel_write_ram(ctrl_pdata);
...
bool mdss_spi_reg_status_check(struct spi_panel_data *ctrl_pdata)
{
	...
	pr_info("%s: Checking Register status\n", __func__);
	...
	if (ret < 0) {
		pr_err("spi esd %s: Read status register returned error\n", __func__);
	} else {
		for (i = 0; i < ctrl_pdata->status_cmds_rlen; i++) {
			pr_info("spi esd act_value[%d] = %x, exp_value[%d] = %x\n",
		...
		struct spi_panel_data	*ctrl)
{
	...
	pr_info("spi esd temp mdss_spi_parse_esd_params enable=%d\n", pinfo->esd_check_enabled);
	...
	if (!rc) {
		if (!strcmp(string, "reg_read")) {
			pr_err("spi esd temp mdss_spi_parse_esd_params   reg_read\n");
			...
			pr_err("spi esd temp mdss_spi_parse_esd_params   send_init_command\n");
				return;
		} else {
	...
	rc = of_property_read_u32(np, "qcom,mdss-spi-panel-status-reg",
				&tmp);
	if (rc) {
	...
	ctrl->panel_status_reg = tmp;/* read_u8 coudl not read the right value*/
	...
	*(ctrl->exp_status_value) = tmp;/* read_u8 coudl not read the right value*/
	...
	if (data) {
		if (!strcmp(data, "bl_ctrl_wled")) {
			...
			if (ctrl_pdata->pwm_pmi) {
				...
				ctrl_pdata->bklt_ctrl = SPI_BL_DCS_CMD;
		} else if (!strcmp(data, "bl_ctrl_gpio")) {
				ctrl_pdata->bklt_ctrl = SPI_BL_CMD_GPIO;
		}
	}
...
static unsigned char spi_led_pwm[] = {0x51, 0xff, 0x2c};

int mdss_spi_panel_bklt_dcs(struct spi_panel_data *ctrl, int level)
{
	...
	spi_led_pwm[1] =  (unsigned char)level;

	mutex_lock(&ctrl->spi_tx_mutex);

	/* pull down dc gpio indicate this is command */
	gpio_set_value(ctrl->disp_dc_gpio, 0);
	mdss_spi_tx_command(&spi_led_pwm[0]);
	gpio_set_value(ctrl->disp_dc_gpio, 1);
	mdss_spi_tx_parameter(&spi_led_pwm[1], 1);
	gpio_set_value(ctrl->disp_dc_gpio, 0);
	mdss_spi_tx_command(&spi_led_pwm[2]);
	gpio_set_value(ctrl->disp_dc_gpio, 1);
...
extern void zte_set_gpio31_pwm_duty(int level);
#endif
static void mdss_spi_panel_bklt_gpio(struct spi_panel_data *ctrl, int level)
{
	pr_info("[MSM_LCD]%s: level=%d\n", __func__, level);
#ifdef ZTE_FEATURE_USE_GPIO_PWM
	if (level >= GPIO_PWM_BL_MIN_LIMIT && level < GPIO_PWM_BL_MAX_LIMIT)
		level = level/2;
	pr_info("[MSM_LCD]%s: new level=%d\n", __func__, level);
	zte_set_gpio31_pwm_duty(level);
#endif
}
							...
							u32 bl_level)
{
	...
	case SPI_BL_PWM:
		...
		break;
	case SPI_BL_DCS_CMD:
		mdss_spi_panel_bklt_dcs(ctrl_pdata, bl_level);
		break;
	case SPI_BL_CMD_GPIO:
		mdss_spi_panel_bklt_gpio(ctrl_pdata, bl_level);
		break;
	default:
...
static int mdss_spi_panel_regulator_init(struct platform_device *pdev)
{
	...
	if (!gpio_is_valid(ctrl_pdata->panel_power_enable)) {
		pr_err("%s:%d, panel_power_enable not configured\n",
			   __func__, __LINE__);
		return rc;
	}
	rc = gpio_request(ctrl_pdata->panel_power_enable, "panel_power_enable");
	if (rc)
		pr_err("panel_power_enable gpio (%d) request failed\n", ctrl_pdata->panel_power_enable);
	else
		pr_info("panel_power_enable gpio (%d) request successful\n", ctrl_pdata->rst_gpio);
				...
				struct spi_panel_data *ctrl_pdata)
{
	...
	ctrl_pdata->panel_power_enable = of_get_named_gpio(ctrl_pdev->dev.of_node,
		"qcom,platform-spi-power-enable-gpio", 0);
	if (!gpio_is_valid(ctrl_pdata->panel_power_enable))
		pr_err("%s:%d, Panel power enable gpio not specified\n",
						__func__, __LINE__);
	...
	if (!gpio_is_valid(ctrl_pdata->rst_gpio))
		pr_err("%s:%d, reset gpio not specified\n",
						__func__, __LINE__);
	pr_info("%s:%d, reset gpio :%d\n",
						__func__,  __LINE__, ctrl_pdata->rst_gpio);
...
static int mdss_spi_panel_probe(struct platform_device *pdev)
{
	...
	pr_info("%s: spi panel  initialized\n", __func__);
	return 0;
