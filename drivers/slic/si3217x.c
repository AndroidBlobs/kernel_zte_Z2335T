...
static struct proslic_cid_work si3217x_cid_data;
static spinlock_t si3217x_cid_spinlock;
...
static void si3217x_fsk_cid_thread(struct kthread_work *cid_work)
{
	struct proslic_cid_work *cid_data = container_of(cid_work, struct proslic_cid_work, cid_work);
	struct proslic_dev *si3217x = cid_data->si3217x;
	struct caller_id_data *cid = cid_data->cid;
	struct slic_dev *slic = &si3217x->sdev;
...
err:
	...
	dev_info(slic->dev, "%s exit result=%d\n", __func__, cid_data->result);

	wake_up(&si3217x->cid_wait);
}
				...
				struct caller_id_data *cid)
{
	...
	si3217x_cid_data.si3217x = si3217x;
	si3217x_cid_data.cid = cid;

	spin_lock_irqsave(&si3217x_cid_spinlock, flags);
	si3217x_cid_data.cid_flag = 0;
	si3217x_cid_data.result = 0;
	queue_kthread_work(&si3217x_cid_data.cid_kworker, &si3217x_cid_data.cid_work);
	spin_unlock_irqrestore(&si3217x_cid_spinlock, flags);

	wait_event(si3217x->cid_wait, si3217x_cid_data.cid_flag != 0);

	dev_info(slic->dev, "%s exit %d\n", __func__, si3217x_cid_data.result);
	return si3217x_cid_data.result;
}
...
static void si3217x_is_onhook(struct work_struct *work)
{
	...
	if (si3217x->state.onhook_timestamp != 0) {
		...
		cancel_delayed_work_sync(&si3217x->dtmf_work);
#endif
	}
}
...
static void si3217x_irq_work(struct kthread_work *irq_work)
{
	struct proslic_work *irq_data = container_of(irq_work, struct proslic_work, irq_work);
...
static irqreturn_t si3217x_irq(int irq, void *dev_id)
{
	...
	queue_kthread_work(&si3217x_irq_data.irq_kworker, &si3217x_irq_data.irq_work);
...
static int slic_irq_init_queue(struct proslic_work *pSi3217x_irq_data)
{
	struct sched_param param = { .sched_priority = MAX_RT_PRIO - 1 };

	init_kthread_worker(&pSi3217x_irq_data->irq_kworker);
	pSi3217x_irq_data->irq_kworker_task = kthread_run(kthread_worker_fn,
					   &pSi3217x_irq_data->irq_kworker, "%s",
					   "slic irq thread");
	if (IS_ERR(pSi3217x_irq_data->irq_kworker_task)) {
		pr_err("%s failed to create irq init task\n", __func__);
		return -ENOMEM;
	}
	init_kthread_work(&pSi3217x_irq_data->irq_work, si3217x_irq_work);
	 ...
	 */
	pr_info("%s will run irq thread with realtime priority\n", __func__);
	sched_setscheduler(pSi3217x_irq_data->irq_kworker_task, SCHED_FIFO, &param);

	return 0;
}

static int slic_irq_destroy_queue(struct proslic_work *pSi3217x_irq_data)
{
	flush_kthread_worker(&pSi3217x_irq_data->irq_kworker);
	kthread_stop(pSi3217x_irq_data->irq_kworker_task);

	return 0;
}

static int slic_cid_init_queue(struct proslic_cid_work *pSi3217x_cid_data)
{
	struct sched_param param = { .sched_priority = MAX_RT_PRIO - 1 };

	init_kthread_worker(&pSi3217x_cid_data->cid_kworker);
	pSi3217x_cid_data->cid_kworker_task = kthread_run(kthread_worker_fn,
					   &pSi3217x_cid_data->cid_kworker, "%s",
					   "slic cid thread");
	if (IS_ERR(pSi3217x_cid_data->cid_kworker_task)) {
		pr_err("%s failed to create cid init task\n", __func__);
		return -ENOMEM;
	}
	init_kthread_work(&pSi3217x_cid_data->cid_work, si3217x_fsk_cid_thread);
	 ...
	 * setting the message pump thread will remain at default priority.
	 */
	pr_info("%s will run cid thread with realtime priority\n", __func__);
	sched_setscheduler(pSi3217x_cid_data->cid_kworker_task, SCHED_FIFO, &param);

	return 0;
}

static int slic_cid_destroy_queue(struct proslic_cid_work *pSi3217x_cid_data)
{
	flush_kthread_worker(&pSi3217x_cid_data->cid_kworker);
	kthread_stop(pSi3217x_cid_data->cid_kworker_task);
...
static int si3217x_spi_probe(struct spi_device *spi)
{
	...
	ret = slic_irq_init_queue(&si3217x_irq_data);
	if (ret) {
		dev_err(&spi->dev, "problem initializing irq queue\n");
		goto err_irq_init_queue;
	}
	si3217x_irq_data.proslic = proslic;
	si3217x_irq_data.irq_err = 0;
	spin_lock_init(&si3217x_irq_spinlock);

	ret = slic_cid_init_queue(&si3217x_cid_data);
	if (ret) {
		dev_err(&spi->dev, "problem initializing cid queue\n");
		goto err_cid_init_queue;
	}
	si3217x_cid_data.proslic = proslic;
	spin_lock_init(&si3217x_cid_spinlock);
	init_waitqueue_head(&si3217x->cid_wait);
...
err_cid_init_queue:
	slic_irq_destroy_queue(&si3217x_irq_data);
err_irq_init_queue:
err_device:
	while(i)
...
static int si3217x_spi_remove(struct spi_device *spi)
{
	...
	slic_irq_destroy_queue(&si3217x_irq_data);
	slic_cid_destroy_queue(&si3217x_cid_data);
	free_irq(irq, proslic);
	free_irq(irq_cp, proslic);
