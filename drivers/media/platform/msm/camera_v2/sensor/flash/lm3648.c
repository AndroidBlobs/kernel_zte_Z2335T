...
int msm_flash_lm3648_led_init(struct msm_led_flash_ctrl_t *fctrl)
{
	...
	if (fctrl->flash_i2c_client) {
		rc = fctrl->flash_i2c_client->i2c_func_tbl->i2c_read(fctrl->flash_i2c_client,
			0x0c, &chipID, MSM_CAMERA_I2C_BYTE_DATA);
		if (rc < 0)
			pr_err("%s:%d read chipID failed\n", __func__, __LINE__);
		}
	pr_err("%s:%d  flash IC chipID is 0x%x\n", __func__, __LINE__, chipID);
	...
	if (fctrl->flash_i2c_client && fctrl->reg_setting) {
		if (chipID == 0x0a) {
			...
			rc = fctrl->flash_i2c_client->i2c_func_tbl->i2c_write(
				fctrl->flash_i2c_client, 0x05, 0x95, MSM_CAMERA_I2C_BYTE_DATA);
			if (rc < 0)
				pr_err("%s:%d failed\n", __func__, __LINE__);
		} else {
			pr_err("%s:%d  flash IC chipID is 0x%x\n", __func__, __LINE__, chipID);
			rc = fctrl->flash_i2c_client->i2c_func_tbl->i2c_write(
				fctrl->flash_i2c_client, 0x05, 0xaf, MSM_CAMERA_I2C_BYTE_DATA);
			if (rc < 0)
			pr_err("%s:%d failed\n", __func__, __LINE__);
		}
		rc = fctrl->flash_i2c_client->i2c_func_tbl->i2c_write_table(
