...
#define ZTE_CONSECUTIVE_COUNT 0
#define NBC1P2_IDEV_CHG_MID 501
#define SLOW_CHG_CAP_THRES 75

void qpnp_detect_dcp_charger_oem_type(struct qpnp_lbc_chip *chip,
	int battery_current, int capacity, int battery_status)
{
	static int count = 0;

	if (chip == NULL) {
		pr_err("pmic fatal error:qpnp_lbc_chip=NULL!!\n");
		return;
	}
...
void qpnp_detect_sdp_charger_oem_type(struct qpnp_lbc_chip *chip,
	int battery_current, int capacity, int battery_status)
{
	static int count = 0;
...
void qpnp_detect_charger_oem_type(struct qpnp_lbc_chip *chip, int battery_current)
{
	int capacity = 0, battery_status = 0;
	int rc = 0;
	...
	if (battery_current > 0) {
		pr_err("battery is not charging!!\n");
		return;
	}
...
static int qpnp_lbc_request_irqs(struct qpnp_lbc_chip *chip)
{
	...
	SPMI_REQUEST_IRQ(chip, USBIN_VALID, rc, usbin_valid, 1,
			IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING
			| IRQF_ONESHOT, 1);
